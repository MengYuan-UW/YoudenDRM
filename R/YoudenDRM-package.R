...
#' @description The YoudenDRM-package contains functions to make inference on the Younde index and the optimal cutoff point under the DRM, including point estimation and confidence interval constrction.
#'
#' @details
#' The Youden index is one of popular summary statistics of the receiver operating characteristic curve.
#' It has the advantage of providing a criterion to choose the “optimal” cutoff.
...
#' \itemize{
#' \item The Duchenne Muscular Dystrophy (\code{\link{DMD}}): the dataset in \insertCite{yuan2021semiparametric;textual}{YoudenDRM}.
#' \item The Density Ratio Model (\code{\link{DRM}}): a function to fit the DRM.
#' \item The point estimation of the Youden index and cutoff point (\code{\link{DRMest}}): a function to estimate the Youden index and cutoff point as well as the asymptotic varinace of these estimators.
#' \item The confidence intervals for the Youden index and cutoff pount (\code{\link{DRMci}}): a function to construct the confidence intervals for the Youden index and cutoff point.
